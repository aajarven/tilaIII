...
!
  ...
  FB = F(T)
  KOUNT = 2
  if (SIGN(1.0D0,FZ)  ==  SIGN(1.0D0,FB)) go to 1
  C = Z
  go to 2
    1 if (Z  ==  C) go to 2
  T = C
  FC = F(T)
  KOUNT = 3
  if (SIGN(1.0D0,FZ)  ==  SIGN(1.0D0,FC)) go to 2
  B = Z
  FB = FZ
    2 A = C
  FA = FC
...
!
  if (SIGN(1.0D0,FB)  /=  SIGN(1.0D0,FC)) go to 3
  C = A
  FC = FA
  go to 3
!
...
!
   10 if (SIGN(1.0D0,FB)  ==  SIGN(1.0D0,FC)) go to 13
