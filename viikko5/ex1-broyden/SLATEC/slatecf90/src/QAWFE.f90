...
!
  REAL A,ABSEPS,ABSERR,ALIST,BLIST,CHEBMO,CORREC,CYCLE, &
    C1,C2,DL,DRL,ELIST,EP,EPS,EPSA,EPSABS,ERLST, &
...
!***FIRST EXECUTABLE STATEMENT  QAWFE
  ...
  if ( (INTEGR /= 1.AND.INTEGR /= 2).OR.EPSABS <= 0.0E+00.OR. &
    LIMLST < 3) IER = 6
  if ( IER == 6) go to 999
  if ( OMEGA /= 0.0E+00) go to 10
!
!           INTEGRATION BY QAGIE if OMEGA IS ZERO
!           --------------------------------------
!
  if ( INTEGR == 1) call QAGIE(F,A,1,EPSABS,0.0E+00,LIMIT, &
...
!
    ...
    (ABSERR <= EPSABS.AND.0.1E+02*CORREC >= EPSABS)) go to 60
...
!
  ...
  if ( PSUM(NUMRL2) == 0.0E+00) go to 999
