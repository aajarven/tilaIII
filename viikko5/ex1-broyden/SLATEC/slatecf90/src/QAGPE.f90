...
!
  ...
  LEVEL(1) = 0
  NPTS = NPTS2-2
  if ( NPTS2 < 2.OR.LIMIT <= NPTS.OR.(EPSABS <= 0.0E+00.AND. &
...
!
  ...
  DO 20 I = 1,NINT
    ...
    DO 20 J = IP1,NINTP1
      ...
      PTS(J) = TEMP
   20 CONTINUE
  if ( PTS(1) /= MIN(A,B).OR.PTS(NINTP1) /=  &
    MAX(A,B)) IER = 6
  if ( IER == 6) go to 999
!
...
!
  ...
  if ( ABSERR <= 0.1E+03*EPMACH*RESABS.AND.ABSERR >  &
    ERRBND) IER = 2
  if ( NINT == 1) go to 80
  DO 70 I = 1,NPTS
    JLOW = I+1
...
!
    ...
    .OR.ERRO12 < 0.99E+00*ERRMAX) go to 90
