...
!  in band form and estimate the condition number of the matrix.
!
...
!
   ...
   50    CONTINUE
     DO 110 K = 1, N
        ...
        if (CABS1(EK-Z(K))  <=  REAL(ABD(M+1,K))) go to 60
...
!
     DO 130 KB = 1, N
        K = N + 1 - KB
        if (CABS1(Z(K))  <=  REAL(ABD(M+1,K))) go to 120
...
!
     DO 150 K = 1, N
        ...
        if (CABS1(Z(K))  <=  REAL(ABD(M+1,K))) go to 140
           S = REAL(ABD(M+1,K))/CABS1(Z(K))
           call CSSCAL(N,S,Z,1)
           YNORM = S*YNORM
  140       CONTINUE
...
!
     DO 170 KB = 1, N
        K = N + 1 - KB
        if (CABS1(Z(K))  <=  REAL(ABD(M+1,K))) go to 160
           S = REAL(ABD(M+1,K))/CABS1(Z(K))
           call CSSCAL(N,S,Z,1)
           YNORM = S*YNORM
  160       CONTINUE
