...
!  by performing an LQ factorization of the matrix using
...
!
  ...
  if ( KEY == 3) go to 300
!
  if ( RE(1) < 0.0) go to 511
  if ( RE(1) > 1.0) go to 512
  if ( RE(1) < EPS) RE(1)=EPS
  if ( AE(1) < 0.0) go to 513
  DO 20 I=1,M
  W(M4-1+I)=RE(1)
  W(M5-1+I)=AE(1)
   20 CONTINUE
...
!
  ...
  if ( AE(1) < 0.0) go to 513
  DO 120 I=1,M
  if ( RE(I) < 0.0) go to 511
  if ( RE(I) > 1.0) go to 512
...
!
  ...
  return
  502 call XERMSG ('SLATEC', 'ULSIA', 'M < 1', 2, 1)
  return
  503 call XERMSG ('SLATEC', 'ULSIA', 'N < 1', 2, 1)
  return
  504 call XERMSG ('SLATEC', 'ULSIA', 'N < M', 2, 1)
  return
  505 call XERMSG ('SLATEC', 'ULSIA', 'MDA < M', 2, 1)
  return
  506 call XERMSG ('SLATEC', 'ULSIA', 'LIW < M+N', 2, 1)
  return
  507 call XERMSG ('SLATEC', 'ULSIA', 'NB < 0', 2, 1)
  return
  508 call XERMSG ('SLATEC', 'ULSIA', 'MDB < N', 2, 1)
  return
  ...
  INFO=-1
  return
  511 call XERMSG ('SLATEC', 'ULSIA', 'RE(I)  <  0', 2, 1)
  return
  512 call XERMSG ('SLATEC', 'ULSIA', 'RE(I)  >  1', 2, 1)
  return
  513 call XERMSG ('SLATEC', 'ULSIA', 'AE(I)  <  0', 2, 1)
  return
