...
20008 NEXT=PRGOPT(LAST)
  if (.NOT.(NEXT <= 0 .OR. NEXT > IADBIG)) go to 20010
!
...
!     WORKING WITH UNDEFINED DATA.
  ...
  INFO=-NERR
  return
20010 if (.NOT.(NEXT == 1)) go to 10001
  go to 20009
...
!     SOLVING.
  if (.NOT.(KEY == 53)) go to 20013
  if (.NOT.(PRGOPT(LAST+2) /= ZERO)) go to 20016
  LMX=PRGOPT(LAST+3)
  LBM=PRGOPT(LAST+4)
20016 CONTINUE
...
!
20009 if (LMX < NVARS+7) THEN
     WRITE (XERN1, '(I8)') LMX
     call XERMSG ('SLATEC', 'DSPLP', 'USER-DEFINED VALUE OF ' // &
        'LAMAT = ' // XERN1 // ' MUST BE  >=  NVARS+7.', 20, 1)
     INFO = -20
     return
  end if
!
...
!
  if (.NOT.(LBM < 0)) go to 20022
  NERR=21
  call XERMSG ('SLATEC', 'DSPLP', &
     'USER-DEFINED VALUE OF LBM MUST BE  >=  0.', NERR, IOPT)
  INFO=-NERR
  return
20022 CONTINUE
!
