...
!***FIRST EXECUTABLE STATEMENT  DXADD
  ...
  go to 220
   40 I = IX - IY
  if (I) 80, 50, 90
   50 if (ABS(X) > 1.0D0 .AND. ABS(Y) > 1.0D0) go to 60
  if (ABS(X) < 1.0D0 .AND. ABS(Y) < 1.0D0) go to 70
  Z = X + Y
  IZ = IX
  go to 220
   60 S = X/RADIXL
...
!
  ...
  if (ABS(T) >= 1.0D0) go to 120
  if (RADIXL*ABS(T) >= 1.0D0) go to 110
  J = I1 + 1
  T = T*RADIX**(L-I2)
  go to 140
  110 J = I1
...
!
  ...
  if (ABS(S) >= 1.0D0) go to (180, 150, 150), J
  if (RADIXL*ABS(S) >= 1.0D0) go to (180, 170, 150), J
  go to (180, 170, 160), J
  150 Z = S
  IZ = IS
  go to 220
...
! SUM.
!
  if (ABS(S) > 1.0D0 .AND. ABS(T) > 1.0D0) go to 200
  if (ABS(S) < 1.0D0 .AND. ABS(T) < 1.0D0) go to 210
  Z = S + T
  IZ = IS - J*L
  go to 220
  200 S = S/RADIXL
