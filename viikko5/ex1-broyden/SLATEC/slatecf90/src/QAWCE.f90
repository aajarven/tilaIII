...
!
  ...
  RESULT = 0.0E+00
  ABSERR = 0.0E+00
  if (C == A.OR.C == B.OR.(EPSABS <= 0.0E+00.AND. &
      EPSREL < MAX(0.5E+02*EPMACH,0.5E-14))) go to 999
!
...
!
  ...
  if ( LIMIT == 1) IER = 1
  if ( ABSERR < MIN(0.1E-01*ABS(RESULT),ERRBND) &
    .OR.IER == 1) go to 70
!
!           INITIALIZATION
!           --------------
...
!
    ...
    if ( ABS(RLIST(MAXERR)-AREA12) < 0.1E-04*ABS(AREA12) &
      .AND.ERRO12 >= 0.99E+00*ERRMAX.AND.KRULE == 0) &
      IROFF1 = IROFF1+1
