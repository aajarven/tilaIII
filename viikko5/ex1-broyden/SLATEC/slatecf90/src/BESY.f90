...
!***FIRST EXECUTABLE STATEMENT  BESY
  ...
  if (FNU < 0.0E0) go to 140
  if (X <= 0.0E0) go to 150
  if (X < XLIM) go to 170
  if (N < 1) go to 160
!
...
!
  100 CONTINUE
!     OVERFLOW TEST
  if (FN <= 1.0E0) go to 110
  if (-FN*(LOG(X)-0.693E0) > ELIM) go to 170
  110 CONTINUE
  if (DNU == 0.0E0) go to 120
