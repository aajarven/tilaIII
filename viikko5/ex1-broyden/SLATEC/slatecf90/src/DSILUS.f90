...
!           I-th row of L
     INDX1 = IL(I)
     INDX2 = IL(I+1) - 1
     if ( INDX1  >  INDX2) go to 200
     DO 190 INDX=INDX1,INDX2
        if ( INDX  ==  INDX1) go to 180
        INDXR1 = INDX1
        INDXR2 = INDX - 1
        INDXC1 = JU(JL(INDX))
        INDXC2 = JU(JL(INDX)+1) - 1
        if ( INDXC1  >  INDXC2) go to 180
 160        KR = JL(INDXR1)
 170        KC = IU(INDXC1)
        if ( KR  >  KC) THEN
           INDXC1 = INDXC1 + 1
...
!         I-th column of U
 200     INDX1 = JU(I)
     INDX2 = JU(I+1) - 1
     if ( INDX1  >  INDX2) go to 260
     DO 250 INDX=INDX1,INDX2
        if ( INDX  ==  INDX1) go to 240
        INDXC1 = INDX1
        INDXC2 = INDX - 1
        INDXR1 = IL(IU(INDX))
        INDXR2 = IL(IU(INDX)+1) - 1
        if ( INDXR1  >  INDXR2) go to 240
 210        KR = JL(INDXR1)
 220        KC = IU(INDXC1)
        if ( KR  >  KC) THEN
           INDXC1 = INDXC1 + 1
...
!         I-th diagonal element
 260     INDXR1 = IL(I)
     INDXR2 = IL(I+1) - 1
     if ( INDXR1  >  INDXR2) go to 300
     INDXC1 = JU(I)
     INDXC2 = JU(I+1) - 1
     if ( INDXC1  >  INDXC2) go to 300
 270     KR = JL(INDXR1)
 280     KC = IU(INDXC1)
     if ( KR  >  KC) THEN
        INDXC1 = INDXC1 + 1
