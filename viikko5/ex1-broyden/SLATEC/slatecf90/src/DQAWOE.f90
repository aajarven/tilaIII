...
!
  ...
  if ( (INTEGR /= 1.AND.INTEGR /= 2).OR.(EPSABS <= 0.0D+00.AND. &
    EPSREL < MAX(0.5D+02*EPMACH,0.5D-28)).OR.ICALL < 1.OR. &
    MAXP1 < 1) IER = 6
  if ( IER == 6) go to 999
!
...
!
  ...
  NUMRL2 = 0
  EXTALL = .FALSE.
  if ( 0.5D+00*ABS(B-A)*DOMEGA > 0.2D+01) go to 10
  NUMRL2 = 1
  EXTALL = .TRUE.
  RLIST2(1) = RESULT
   10 if ( 0.25D+00*ABS(B-A)*DOMEGA <= 0.2D+01) EXTALL = .TRUE.
  KSGN = -1
...
!
    ...
    .OR.ERRO12 < 0.99D+00*ERRMAX) go to 20
...
!
  ...
  200 if (INTEGR == 2.AND.OMEGA < 0.0D+00) RESULT=-RESULT
  999 RETURN
end
