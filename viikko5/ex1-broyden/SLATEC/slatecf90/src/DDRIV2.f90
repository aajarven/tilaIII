subroutine DDRIV2 (N, T, Y, F, TOUT, MSTATE, NROOT, EPS, EWT, &
     MINT, WORK, LENW, IWORK, LENIW, G, IERFLG)
!
!! DDRIV2 solves N ordinary differential equations of the form ...
!  dY(I)/dT = F(Y(I),T), given the initial conditions Y(I) = YI.  ...
!  The program has options to allow the solution of both stiff and ...
!  non-stiff differential equations.  DDRIV2 uses double precision arithmetic.
!
...
!***FIRST EXECUTABLE STATEMENT  DDRIV2
  if (ABS(MSTATE)  ==  9) THEN
    ...
    'Illegal input.  The magnitude of MSTATE IS 9 .', &
    IERFLG, 2)
    return
  ELSE if (ABS(MSTATE)  ==  0 .OR. ABS(MSTATE)  >  9) THEN
  ...
  call DDRIV3 (N, T, Y, F, NSTATE, TOUT, NTASK, NROOT, EPS, EWTCOM, &
               ...
               LENW, IWORK, LENIW, F, F, NDE, MXSTEP, G, F, IERFLG)
  if (NSTATE  <=  7) THEN
    MSTATE = SIGN(NSTATE, MSTATE)
  ELSE if (NSTATE  ==  11) THEN
