...
!***END PROLOGUE  SDRIV3
  ...
  REAL AE, BIG, EPS, EWT(*), G, GLAST, GNOW, H, HMAX, &
       HSIGN, HUSED, NROUND, RE, R1MACH, SIZE, SNRM2, SUM, T, TLAST, &
...
!
  call SDSTP (EPS, F, FA, WORK(IHMAX), IMPL, IERROR, JACOBN, &
              MATDIM, IWORK(IMXORD), IWORK(IMNT), IWORK(IMTR), ML, &
              MU, NPAR, NDECOM, WORK(IYWT), UROUND, USERS, &
              WORK(IAVGH), WORK(IAVGRD), WORK(IH), HUSED, &
              IWORK(IJTASK), IWORK(IMNTLD), IWORK(IMTRLD), &
              IWORK(INFE), IWORK(INJE), IWORK(INQUSE), &
              IWORK(INSTEP), WORK(IT), Y, WORK(IYH), WORK(IA), &
              CONVRG, WORK(IDFDY), WORK(IEL), WORK(IFAC), &
              WORK(IHOLD), IWORK(INDPVT), JSTATE, IWORK(IJSTPL), &
              IWORK(INQ), IWORK(INWAIT), WORK(IRC), WORK(IRMAX), &
              WORK(ISAVE1), WORK(ISAVE2), WORK(ITQ), WORK(ITREND), &
