...
!     CHECK RTOL,ATOL
  ...
  DO 210 I=1,NEQ
     ...
     if ( RTOLI < 0.0E0)go to 706
     if ( ATOLI < 0.0E0)go to 707
210      CONTINUE
...
!     SET ERROR WEIGHT VECTOR WT
  ...
  DO 305 I = 1,NEQ
     if ( RWORK(LWT+I-1) <= 0.0E0) go to 713
305      CONTINUE
!
...
!     CHECK HO, if THIS WAS INPUT
  if (INFO(8)  ==  0) go to 310
     HO = RWORK(LH)
     if ((TOUT - T)*HO  <  0.0E0) go to 711
     if (HO  ==  0.0E0) go to 712
     go to 320
310    CONTINUE
!
...
330   if (INFO(4)  ==  0) go to 340
     TSTOP = RWORK(LTSTOP)
     if ((TSTOP - T)*HO  <  0.0E0) go to 715
     if ((T + HO - TSTOP)*HO  >  0.0E0) HO = TSTOP - T
     if ((TSTOP - TOUT)*HO  <  0.0E0) go to 709
!
...
410   CONTINUE
  if ( T  ==  TOUT) go to 719
  if ( (T - TOUT)*H  >  0.0E0) go to 711
  if ( INFO(4)  ==  1) go to 430
  if ( INFO(3)  ==  1) go to 420
  if ( (TN-TOUT)*H < 0.0E0)go to 490
  ...
  DONE = .TRUE.
  go to 490
420   if ( (TN-T)*H  <=  0.0E0) go to 490
  if ( (TN - TOUT)*H  >  0.0E0) go to 425
...
430   if ( INFO(3)  ==  1) go to 440
  TSTOP=RWORK(LTSTOP)
  if ( (TN-TSTOP)*H > 0.0E0) go to 715
  if ( (TSTOP-TOUT)*H < 0.0E0)go to 709
  if ( (TN-TOUT)*H < 0.0E0)go to 450
  call SDATRP(TN,TOUT,Y,YPRIME,NEQ,IWORK(LKOLD), &
     RWORK(LPHI),RWORK(LPSI))
  T=TOUT
  IDID = 3
...
440   TSTOP = RWORK(LTSTOP)
  if ( (TN-TSTOP)*H  >  0.0E0) go to 715
  if ( (TSTOP-TOUT)*H  <  0.0E0) go to 709
  if ( (TN-T)*H  <=  0.0E0) go to 450
  if ( (TN - TOUT)*H  >  0.0E0) go to 445
  call SDATRP(TN,TN,Y,YPRIME,NEQ,IWORK(LKOLD), &
    RWORK(LPHI),RWORK(LPSI))
  T = TN
  IDID = 1
...
!     CHECK WHETHER WE ARE WITHIN ROUNDOFF OF TSTOP
  if ( ABS(TN-TSTOP) > 100.0E0*UROUND* &
  ...
  go to 490
460   TNEXT=TN+H
  if ( (TNEXT-TSTOP)*H <= 0.0E0)go to 490
  H=TSTOP-TN
  RWORK(LH)=H
!
...
510   call SDAWTS(NEQ,INFO(2),RTOL,ATOL,RWORK(LPHI), &
    RWORK(LWT),RPAR,IPAR)
  DO 520 I=1,NEQ
     if ( RWORK(I+LWT-1) > 0.0E0)go to 520
       IDID=-3
       go to 527
520   CONTINUE
!
...
540   if ( INFO(3) /= 0)go to 550
  if ( (TN-TOUT)*H < 0.0E0)go to 542
     call SDATRP(TN,TOUT,Y,YPRIME,NEQ, &
       IWORK(LKOLD),RWORK(LPHI),RWORK(LPSI))
     T=TOUT
     IDID=3
     go to 580
542   if ( ABS(TN-TSTOP) <= 100.0E0*UROUND* &
     (ABS(TN)+ABS(H)))go to 545
  TNEXT=TN+H
  if ( (TNEXT-TSTOP)*H <= 0.0E0)go to 500
  H=TSTOP-TN
  go to 500
545   call SDATRP(TN,TSTOP,Y,YPRIME,NEQ, &
  ...
  T=TSTOP
  go to 580
550   if ( (TN-TOUT)*H >= 0.0E0)go to 555
  if ( ABS(TN-TSTOP) <= 100.0E0*UROUND*(ABS(TN)+ABS(H)))go to 552
  T=TN
  IDID=1
  go to 580
