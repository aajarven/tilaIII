...
!
  if (IOPT  <  1 .OR. IOPT  >  3 .OR. &
      N  <=  0 .OR. M  <  N .OR. TOL  <  ZERO &
      .OR. LWA  <  N*(N+5) + M) go to 10
  if (IOPT  <  3 .AND. LWA  <  N*(M+5) + M) go to 10
!
!     call DNLS1.
!
  MAXFEV = 100*(N + 1)
  ...
  call DNLS1(FCN,IOPT,M,N,X,FVEC,WA(INDEX+1),M,FTOL,XTOL,GTOL, &
             ...
             IW,WA(N+1),WA(2*N+1),WA(3*N+1),WA(4*N+1),WA(5*N+1))
  if (INFO  ==  8) INFO = 4
   10 CONTINUE
  if (INFO  ==  0) call XERMSG ('SLATEC', 'DNLS1E', &
